...
import type { WeeklyWorkout } from "@/lib/mockApi";
...
const normalize = (rows: WeeklyWorkout[]): WeeklyWorkout[] => {
  return rows.map((row) => {
    const total = row.running + row.cycling + row.stretching || 1;
    return {
      ...row,
      running: (row.running / total) * 100,
      cycling: (row.cycling / total) * 100,
      stretching: (row.stretching / total) * 100,
    };
  });
};

interface WeeklyWorkoutChartsProps {
  data: WeeklyWorkout[];
}

export const WeeklyWorkoutCharts = ({ data }: WeeklyWorkoutChartsProps) => {
  const [legend, setLegend] = useState<LegendItem[]>([
    { key: "running", label: "러닝", color: COLORS[0], visible: true },
    { key: "cycling", label: "사이클", color: COLORS[1], visible: true },
    { key: "stretching", label: "스트레칭", color: COLORS[2], visible: true },
  ]);
  ...
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <p className="mb-4 text-xs text-gray-500">
        /mock/weekly-workout-trend 데이터를 활용한 100% 스택형 바/면적 차트
      </p>
